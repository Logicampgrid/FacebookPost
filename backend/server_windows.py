...
async def exchange_facebook_code(code: str, redirect_uri: str) -> dict:
    ...
    try:
        ...
        if not redirect_uri or "localhost" in redirect_uri:
            ...
            if active_ngrok_url:
                redirect_uri = f"{active_ngrok_url}/auth/callb"  # Chemin complet avec callback
...
async def sync_ngrok_urls():
    ...
    try:
        ...
        test_redirect_uri = build_dynamic_redirect_uri("/")
...
async def upload_video_endpoint(video: UploadFile = File(...)):
    ...
    try:
        ...
        # PATCH 24: Conservation fichier temporaire pour accès FB/IG
        # Les fichiers seront nettoyés par une tâche périodique ou manuellement
        try:
            # os.remove(temp_path)
            log_app(f"📁 PATCH 24: Fichier vidéo conservé: {temp_path}", "INFO")
        except:
            pass
