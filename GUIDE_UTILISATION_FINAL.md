...
### **Interface Web**
1. Connectez-vous à https://auth-key-manager.preview.emergentagent.com
...
### **API Directe**
```bash
GET https://auth-key-manager.preview.emergentagent.com/api/webhook-history?limit=100
```
...
```bash
curl -s "https://auth-key-manager.preview.emergentagent.com/api/publishProduct/config" | jq '.shop_types'
```
