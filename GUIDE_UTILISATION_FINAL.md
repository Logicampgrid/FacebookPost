...
### **Interface Web**
1. Connectez-vous à https://auth-token-fix-5.preview.emergentagent.com
...
### **API Directe**
```bash
GET https://auth-token-fix-5.preview.emergentagent.com/api/webhook-history?limit=100
```
...
```bash
curl -s "https://auth-token-fix-5.preview.emergentagent.com/api/publishProduct/config" | jq '.shop_types'
```
