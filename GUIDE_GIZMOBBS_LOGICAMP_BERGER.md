...
### **Endpoint Principal**
```
POST https://auth-relay-1.preview.emergentagent.com/api/webhook
```

### **Format Multipart/Form-Data**
```bash
curl -X POST "https://auth-relay-1.preview.emergentagent.com/api/webhook" \
...
```bash
curl -X POST "https://auth-relay-1.preview.emergentagent.com/api/publishProduct" \
...
{
  "method": "POST",
  "url": "https://auth-relay-1.preview.emergentagent.com/api/publishProduct",
  "headers": {
...
```bash
curl -X POST "https://auth-relay-1.preview.emergentagent.com/api/debug/test-logicamp-berger-webhook"
```

### **Test Configuration Gizmobbs**
```bash
curl -X POST "https://auth-relay-1.preview.emergentagent.com/api/debug/test-multi-platform-post?shop_type=gizmobbs"
```

### **Diagnostic Instagram Complet**
```bash
curl -X GET "https://auth-relay-1.preview.emergentagent.com/api/debug/instagram-complete-diagnosis"
```
...
1. **Se connecter via l'interface web :**
   ```
   https://auth-relay-1.preview.emergentagent.com
   ```
...
1. **Vérifier l'authentification :**
   ```bash
   curl -X GET "https://auth-relay-1.preview.emergentagent.com/api/health"
